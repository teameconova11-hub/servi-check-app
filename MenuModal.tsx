...
interface MenuModalProps {
  open: boolean;
  editingMenu: { id: string; name: string } | null;
  onClose: () => void;
  onSave: (name: string) => Promise<void>;
}

export default function MenuModal({ open, editingMenu, onClose, onSave }: MenuModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-800">
                {editingMenu ? 'Editar carta' : 'Nueva carta'}
              </h3>
              <p className="text-xs text-slate-500">
                {editingMenu ? 'Modifica el nombre de la carta' : 'Crea una nueva carta o menú'}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-slate-600">Nombre de la carta</label>
            <input
              type="text"
              value={name}
